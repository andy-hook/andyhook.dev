        ...
        <div className="rounded-3xl shadow-slate-1 shadow-2xl border border-slate-3 from-slate-3/30 to-slate-2/30 bg-gradient-to-br relative">
          <HoverGroup.Root value={hoveredItem} onValueChange={setHoveredItem}>
            <Accordion.Root
              ...
              onValueChange={(values) => onOpenItemChange(values[0] ?? '')}
              render={<ul />}
            >
              {experience.map((entry, i) => {
                ...
                return (
                  <Accordion.Item value={key} key={key} className="relative" render={<li />}>
                    {(open || hovered) && (
                      <div
                        className={cx(
                    ...
                    <Accordion.Header render={<div />}>
                      <FocusRing className="-outline-offset-4 focus-visible:-outline-offset-2">
                        <ExperienceListItemTrigger
                          open={open}
                          className={cx(
                            'group grid grid-cols-[auto_auto_1fr_auto] sm:grid-cols-[auto_auto_1fr_auto] md:grid-cols-[4.25rem_3.25rem_1fr_7.5rem] lg:grid-cols-[5.25rem_3.25rem_1fr_8.5rem] gap-y-2 px-5 md:px-7 xl:px-10 py-5 xl:py-6 items-center relative w-full text-left rounded-xl',
                            firstItem && 'rounded-t-3xl',
                            lastItem && 'rounded-b-3xl',
                          )}
                          ...
                          {({ yearScrambleRef, companyScrambleRef, titleScrambleRef }) => (
                            <>
                              ...
                              <span className="grid grid-cols-1 md:grid-cols-2 grid-rows-2 md:grid-rows-1 gap-y-2 md:items-center col-start-3 sm:col-start-auto sm:row-start-auto pt-1 md:pt-0">
                                <h3
                                  className={cx(
                                    'text-sm sm:text-base md:text-lg xxl:text-xl text-slate-12 font-body capsize col-span-2 md:col-span-1 font-medium duration-250 transition-[transform,color] ease-gentle',
                                    animateCompanyName && open && 'translate-y-[0.5em]',
                                  )}
                                  style={{
                                ...
                                <p
                                  className={cx(
                                    'text-xs md:text-sm lg:text-base font-body text-slate-10 font-normal capsize col-start-1 col-span-2 md:col-span-1 md:col-start-2 transition-[opacity,transform] duration-250 ease-gentle',
                                    'group-data-[panel-open]:opacity-0 group-data-[panel-open]:translate-y-full group-data-[panel-open]:pointer-events-none',
                                  )}
                                  aria-hidden={open}
                                >
                                  <ScrambleText ref={titleScrambleRef}>{entry.title}</ScrambleText>
                                </p>
                              </span>
                              ...
                              <div className="flex justify-end col-start-4">
                                <div className="p-1.5 md:p-2 border border-slate-7 rounded-full bg-gradient-to-br from-slate-3">
                                  <div className="scale-75 transition-transform duration-250 ease-gentle group-data-[panel-open]:rotate-45 group-data-[panel-open]:scale-100">
                    ...
                    <Accordion.Panel
                      className={cx(
                        'group/panel h-[var(--accordion-panel-height)] overflow-clip',
                        'transition-[height] duration-250 ease-gentle',
                        'data-[starting-style]:h-0 data-[ending-style]:h-0',
                      )}
                    >
                      <div className={cx('px-5 md:px-7 xl:px-10 relative overflow-hidden')}>
                        <div
                          className={cx(
                            'md:pl-[4.25rem] lg:pl-[5.25rem] pb-14 pt-2 relative',
                            'group-data-[starting-style]/panel:opacity-0 group-data-[starting-style]/panel:translate-y-5',
                            'group-data-[ending-style]/panel:opacity-0 group-data-[ending-style]/panel:translate-y-5',
...
type ExperienceListItemTriggerElement = React.ComponentRef<typeof Accordion.Trigger>;
